...
class Tester(object):
    def __init__(self,valid_data_loader):
        ...
        ## self.pretrained_model = 82
        self.pretrained_model = 43
        # self.pretrained_model = 36
        self.total_step = 100

        # Path
        self.model_save_path = "p2/final"
    ...
    def _run_epoch(self, training, out_file):
        ...
        import csv
        submission = open(os.path.join(out_file, "p2_result.txt"), "w+") # "./{}.csv".format(target_dataset_name)

        with open(os.path.join(out_file, "p2_result.txt"), "w+") as f:
            for p in output:
                f.write(str(p)+'\n')
    ...
    def build_model(self):
        ...
        # Loss and optimizer
        self.optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, self.classifier.parameters()), 1e-5)
    ...
    def load_pretrained_model(self):
        ...
        self.model.load_state_dict(torch.load(os.path.join(
            self.model_save_path, '{}_model.pth'.format("encode"))))
