...
class Self_Attn(nn.Module):
    ...
    def forward(self,x):
        ...
        proj_query  = self.query_linear(x).view(m_batchsize,S,-1).permute(0,2,1) # B X CX(N)
        proj_key =  self.key_linear(x).view(m_batchsize,S,-1) # B X C x (*W*H)
...
class Classifier(nn.Module):
    ...
    def __init__(self):
        ...
        self.n_layers = 2
        dropout = 0.5
        self.lstm = nn.LSTM(2048, self.hidden_size, self.n_layers,
